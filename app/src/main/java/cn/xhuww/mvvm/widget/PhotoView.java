...
//            mScaleCenter.set(detector.getFocusX(), detector.getFocusY());
    ...
    public PhotoImaInfo getInfo() {
        ...
        return new PhotoImaInfo(rect, local, mImgRect, mWidgetRect, mScale, mDegrees, mScaleType);
    }
     ...
     */
    public void animaFrom(PhotoImaInfo info) {

        if (isInit) {
            reset();

            PhotoImaInfo mine = getInfo();
    ...
    public void animaTo(PhotoImaInfo info, Runnable completeCallBack) {
        if (isInit) {
            mTranslate.stop();
